"""YMODEM-1K sender used by the RX261 OTA runner."""
...
def _response(transport, accepted, timeout):
    deadline = time.monotonic() + timeout
    while time.monotonic() < deadline:
        data = transport.read(1, deadline - time.monotonic())
        if data and data[0] in accepted:
            return data[0]
    return -1


def _packet(transport, sequence, payload, use_1k):
    size = 1024 if use_1k else 128
    payload = payload[:size].ljust(size, b"\0")
    transport.write(bytes((STX if use_1k else SOH, sequence, 0xFF - sequence)) +
                    payload + struct.pack(">H", crc16_ccitt(payload)))
    transport.flush()


def ymodem_send(transport, data, filename):
    """Send one file over transport and return True after receiver completion."""
    if _response(transport, (CRC,), 20) != CRC:
        return False
    _packet(transport, 0, filename.encode("ascii", "replace") + b"\0" + str(len(data)).encode() + b"\0", False)
